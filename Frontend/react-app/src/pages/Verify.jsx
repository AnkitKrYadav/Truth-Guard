...
import { postVerify } from "../utils/api";
...
function Verify() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.error || err?.message || "Error verifying claim";
      setResult({ error: msg });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen">
      <h2 className="text-3xl font-bold mb-6">Verify News</h2>
      ...
      {result && (
        <div className="p-5 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm bg-white dark:bg-gray-800 max-w-2xl">
          {result.error ? (
            ...
            <>
              ...
              {result.sources?.length > 0 && (
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                  <span className="font-medium">Sources:</span> {result.sources.join(", ")}
                </p>
              )}
            </>
          )}
